...
struct ObjectParameters {
    // Common parameters
    double width;       // X dimension (for bin) or diameter (for cylinder)
    double depth;       // Y dimension (for bin) or diameter (for cylinder) 
    double height;      // Z dimension
    double wall_thickness; // Wall thickness or spoke thickness
...
public:
    ...
    static moveit_msgs::msg::CollisionObject createBin(const ObjectParameters& params) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("object_factory"), "Created bin collision object with 5 walls");
        return bin_object;
    }
    ...
    static moveit_msgs::msg::CollisionObject createCylinderWithSpokes(const ObjectParameters& params) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("object_factory"), "Created cylinder with 4 spokes collision object");
    ...
    static moveit_msgs::msg::CollisionObject createObject(ObjectType type, const ObjectParameters& params) {
        switch (type) {
            ...
            default:
                RCLCPP_ERROR(rclcpp::get_logger("object_factory"), "Unknown object type, returning empty collision object");
