...
int main(int argc, char * argv[]){
  ...
  const moveit::core::JointModelGroup* left_arm_group = kinematic_model->getJointModelGroup("left_arm");
  RCLCPP_INFO(logger, "Waiting for move_group capability...");
  ...
  bool success_left = kinematic_state->setFromIK(
    left_arm_group, goal, 0.1,  // timeout
    moveit::core::GroupStateValidityCallbackFn());
  if (success_left)
  {
    std::vector<double> joint_values;
    kinematic_state->copyJointGroupPositions(left_arm_group, joint_values);
    RCLCPP_INFO(logger, "Left arm IK solution:");
    for (size_t i = 0; i < joint_values.size(); ++i)
    {
      RCLCPP_INFO(logger, "  Joint %lu: %f", i + 1, joint_values[i]);
    }
  }
  else
  {
    RCLCPP_ERROR(logger, "Failed to find IK solution for left arm.");
  }
